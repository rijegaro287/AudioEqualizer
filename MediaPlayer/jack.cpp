...
int jack::process(jack_nframes_t nframes, void *arg)
{
  ...
  {
    in = static_cast<jack_default_audio_sample_t *>(jack_port_get_buffer(inputPort_, nframes));
  }
  out = static_cast<jack_default_audio_sample_t *>(jack_port_get_buffer(outputPort_, nframes));

  // return 0 on success, or anything else on error
  return (dsp_->process(in, out)) ? 0 : 1;
}
